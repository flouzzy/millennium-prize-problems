...
theorem navier_stokes_energy_decay (E₀ ν : ℝ) (hE : E₀ > 0) (hν : ν > 0) (t : ℝ) (ht : t ≥ 0) :
    ...
    E_t ≤ E₀ ∧ E_t > 0 := by
  dsimp
  have h_exp_neg : -2 * ν * t ≤ 0 := mul_nonpos_of_nonpos_of_nonneg (by linarith) ht
  have h_exp_le_one : Real.exp (-2 * ν * t) ≤ 1 := by
    rw [Real.exp_le_one_iff]
    exact h_exp_neg
  have h_exp_pos : Real.exp (-2 * ν * t) > 0 := Real.exp_pos _
  refine ⟨?_, ?_⟩
  · have h_mul : E₀ * Real.exp (-2 * ν * t) ≤ E₀ * 1 := mul_le_mul_of_nonneg_left h_exp_le_one (by linarith)
    linarith
  · positivity
