...
theorem navier_stokes_energy_decay (E₀ ν : ℝ) (hE : E₀ > 0) (hν : ν > 0) (t : ℝ) (ht : t ≥ 0) :
    ...
    E_t ≤ E₀ ∧ E_t > 0 := by
  dsimp
  have h_neg_two : (-2 : ℝ) ≤ 0 := by linarith
  have h_mul_1 : -2 * ν ≤ 0 := mul_nonpos_of_nonpos_of_nonneg h_neg_two (le_of_lt hν)
  have h_exp_neg : -2 * ν * t ≤ 0 := mul_nonpos_of_nonpos_of_nonneg h_mul_1 ht
  have h_exp_le_one : Real.exp (-2 * ν * t) ≤ 1 := by
    rw [Real.exp_le_one_iff]
    exact h_exp_neg
  have h_exp_pos : Real.exp (-2 * ν * t) > 0 := Real.exp_pos _
  refine ⟨?_, ?_⟩
  · exact mul_le_of_le_one_right (le_of_lt hE) h_exp_le_one
  · positivity
