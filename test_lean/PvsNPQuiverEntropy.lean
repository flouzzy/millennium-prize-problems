...
theorem circuit_lower_bound_strict (n : ℕ) (hn : n ≥ 5) :
  ...
  · push_cast
    ...
    have h_quad : (k + 1 : ℝ) ^ 2 < (k : ℝ) ^ 2 * 2 := by
      have hk_cast : (k : ℝ) ≥ 5 := by exact_mod_cast hk
      have h1 : 2 * (k : ℝ) + 1 < (k : ℝ) ^ 2 := by
        calc
          2 * (k : ℝ) + 1 ≤ 2 * (k : ℝ) + (k : ℝ) := by linarith
          _ = 3 * (k : ℝ) := by ring
          _ < 5 * (k : ℝ) := by linarith
          _ ≤ (k : ℝ) * (k : ℝ) := mul_le_mul_of_nonneg_right hk_cast (by linarith)
          _ = (k : ℝ) ^ 2 := by ring
      calc
        (k + 1 : ℝ) ^ 2 = (k : ℝ) ^ 2 + 2 * (k : ℝ) + 1 := by ring
        _ < (k : ℝ) ^ 2 + (k : ℝ) ^ 2 := by linarith
        _ = (k : ℝ) ^ 2 * 2 := by ring
    linarith
