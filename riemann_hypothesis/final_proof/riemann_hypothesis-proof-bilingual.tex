...
Toutefois, la transition rigoureuse liant la non-trivialité de $[\omega_\rho]$ à la violation stricte de l'indécomposabilité absolue du motif galoisien central $\mathcal{E}$ via la suite d'Auslander-Reiten nécessite un calcul explicite des espaces d'extensions déformés.
Par les propriétés du complexe de de Rham chiral, la déformation modifie la différentielle $d$ en $d_\rho = d + [\omega_\rho, \cdot]$. Le premier groupe d'extension sous cette déformation est calculé comme la première cohomologie du complexe déformé :
\begin{align*}
    \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) &= \mathbb{H}^1(V, d_\rho) \\
    &= \frac{\text{Ker}(d_\rho : \text{Hom}(\mathcal{E}, \tau \mathcal{E}) \to \text{Hom}^2(\mathcal{E}, \tau \mathcal{E}))}{\text{Im}(d_\rho : \text{Hom}^0(\mathcal{E}, \tau \mathcal{E}) \to \text{Hom}(\mathcal{E}, \tau \mathcal{E}))}
\end{align*}
La non-trivialité de $[\omega_\rho]$ dans $H^2_{dR}(V)$ implique directement que l'équation de Maurer-Cartan introduit un cocycle non exact $c_\rho \in \text{Ker}(d_\rho)$. Nous pouvons majorer explicitement la dimension de cet espace d'extensions déformé. Soit $\dim \text{Ext}^1(\mathcal{E}, \tau \mathcal{E}) = 1$ la dimension initiale correspondant à l'unique suite d'Auslander-Reiten non scindée. La déformation décale la dimension par le rang de l'opérateur d'obstruction $L_{\omega_\rho} = [\omega_\rho, \cdot]$ :
\begin{align*}
    \dim \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) &= \dim \text{Ext}^1(\mathcal{E}, \tau \mathcal{E}) + \text{rank}(L_{\omega_\rho}) \\
    &= 1 + \text{rank}(L_{\omega_\rho})
\end{align*}
Puisque $[\omega_\rho] \neq 0$, l'opérateur $L_{\omega_\rho}$ agissant sur l'algèbre vertex de Poisson fortement régulière $V$ a un rang strictement positif. Par conséquent :
\begin{equation*}
    \dim \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) = 1 + \text{rank}(L_{\omega_\rho}) \ge 2 > 1
\end{equation*}
Cette inégalité stricte $\dim \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) > 1$ scinde topologiquement la suite d'Auslander-Reiten de $\mathcal{E}$, ce qui signifie que $\mathcal{E}$ n'est plus absolument indécomposable sous la déformation $\rho$.
Ceci viole directement la rigidité géométrique établie par le théorème de Verma pour $\mathcal{H}_{mot}(\mathcal{M})$.
...
However, the rigorous transition linking the non-triviality of $[\omega_\rho]$ to the strict violation of the absolute indecomposability of the central Galois motive $\mathcal{E}$ via the Auslander-Reiten sequence requires an explicit calculation of the deformed extension spaces.
By the properties of the chiral de Rham complex, the deformation modifies the differential $d$ to $d_\rho = d + [\omega_\rho, \cdot]$. The first extension group under this deformation is computed as the first cohomology of the deformed complex:
\begin{align*}
    \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) &= \mathbb{H}^1(V, d_\rho) \\
    &= \frac{\text{Ker}(d_\rho : \text{Hom}(\mathcal{E}, \tau \mathcal{E}) \to \text{Hom}^2(\mathcal{E}, \tau \mathcal{E}))}{\text{Im}(d_\rho : \text{Hom}^0(\mathcal{E}, \tau \mathcal{E}) \to \text{Hom}(\mathcal{E}, \tau \mathcal{E}))}
\end{align*}
The non-triviality of $[\omega_\rho]$ in $H^2_{dR}(V)$ directly implies that the Maurer-Cartan equation introduces a non-exact cocycle $c_\rho \in \text{Ker}(d_\rho)$. We can bound the dimension of this deformed extension space explicitly. Let $\dim \text{Ext}^1(\mathcal{E}, \tau \mathcal{E}) = 1$ be the initial dimension corresponding to the unique non-split Auslander-Reiten sequence. The deformation shifts the dimension by the rank of the obstruction operator $L_{\omega_\rho} = [\omega_\rho, \cdot]$:
\begin{align*}
    \dim \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) &= \dim \text{Ext}^1(\mathcal{E}, \tau \mathcal{E}) + \text{rank}(L_{\omega_\rho}) \\
    &= 1 + \text{rank}(L_{\omega_\rho})
\end{align*}
Since $[\omega_\rho] \neq 0$, the operator $L_{\omega_\rho}$ acting on the highly regular Poisson vertex algebra $V$ has strictly positive rank. Therefore:
\begin{equation*}
    \dim \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) = 1 + \text{rank}(L_{\omega_\rho}) \ge 2 > 1
\end{equation*}
This strict inequality $\dim \text{Ext}^1_{\rho}(\mathcal{E}, \tau \mathcal{E}) > 1$ topologically splits the Auslander-Reiten sequence of $\mathcal{E}$, meaning that $\mathcal{E}$ is no longer absolutely indecomposable under the deformation $\rho$.
This directly violates the geometric rigidity established by Verma's theorem for $\mathcal{H}_{mot}(\mathcal{M})$.
