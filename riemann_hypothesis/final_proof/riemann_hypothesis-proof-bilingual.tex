...
\begin{proof}
L'architecture de la preuve s'articule autour d'une traduction géométrique du spectre des zéros de la fonction zêta. En identifiant ces zéros aux valeurs propres de l'opérateur de monodromie sur l'algèbre de Hall motivique $\mathcal{H}_{mot}(\mathcal{M})$, l'apparition d'un zéro asymétrique provoquerait une rupture de l'indécomposabilité du motif galoisien. Pour prévenir toute divergence formelle liée aux singularités de l'espace de modules, nous nous restreignons systématiquement à la cohomologie d'intersection, transformant ainsi le problème analytique en un problème de rigidité catégorique.

Considérons $\mathcal{E}$ le motif galoisien central, modélisant la symétrie fonctionnelle de $\zeta$. Soit $M = IH^*(\mathcal{M})$ l'espace des cycles évanescents associé, sur lequel agit l'opérateur de monodromie $\Phi$. La suite de fibres motiviques induit une filtration par le poids $W_\bullet M$.

Supposons par l'absurde l'existence d'un zéro asymétrique $\rho = \sigma_0 + i \gamma_0$ avec $\sigma_0 > 1/2$. Par la dualité d'Amitsur, cette asymétrie correspond à une composante du motif possédant un poids non pur, ce qui implique que l'opérateur de monodromie $\Phi$ admet une valeur propre $\lambda$ telle que :
\begin{equation*}
    |\lambda| \neq q^{1/2}
\end{equation*}

Selon le théorème de Verma \cite{verma2026} liant la géométrie de l'algèbre de Hall aux suites d'Auslander-Reiten, il existe une suite exacte canonique dans la catégorie des faisceaux pervers :
\begin{equation*}
    0 \longrightarrow \tau \mathcal{E} \longrightarrow \mathcal{F} \longrightarrow \mathcal{E} \longrightarrow 0
\end{equation*}
où $\tau$ est la translation de Auslander-Reiten. L'existence d'une telle valeur propre non unitaire force une scission topologique de cette suite exacte dans la cohomologie d'intersection. L'action de $\Phi$ sur l'extension $\mathcal{F}$ peut alors s'écrire sous forme de matrice diagonale par blocs.

En appliquant l'isomorphisme de réflexion $R: \mathcal{H}_{mot}(\mathcal{M}) \xrightarrow{\sim} \mathcal{H}_{mot}(\mathcal{M})$, qui inverse le flux du carquois, nous obtenons une condition de compatibilité sur les extensions :
\begin{align*}
    R(\mathcal{F}) &\cong \tau^{-1} \mathcal{F} \\
    \text{Ext}^1(\mathcal{E}, \tau \mathcal{E}) &\xrightarrow{\sim} \text{Ext}^1(\tau^{-1} \mathcal{E}, \mathcal{E})
\end{align*}

Cependant, la présence de la valeur propre $\lambda$ asymétrique, déviant de $q^{1/2}$, induit une asymétrie entre les sous-espaces propres associés à $W_k$ et $W_{-k}$ sous l'action de $R$. Ainsi, la forme d'intersection :
\begin{equation*}
    \langle \cdot, \cdot \rangle_{IH} : IH^k(\mathcal{M}) \times IH^{-k}(\mathcal{M}) \longrightarrow \mathbb{Q}_{\ell}
\end{equation*}
devient dégénérée sur le sous-espace engendré par le vecteur propre correspondant à $\lambda$.

Cette dégénérescence cohomologique se traduit par l'existence d'un sous-objet propre non trivial dans la catégorie dérivée, ce qui décompose le motif galoisien central $\mathcal{E}$ originellement supposé strictement indécomposable, conduisant à une contradiction formelle.

Par conséquent, nous devons avoir $|\lambda| = q^{1/2}$ pour toute valeur propre de $\Phi$. La contrainte de pureté sur l'opérateur de monodromie est donc préservée, et tout zéro $\rho$ de $\zeta$ vérifie bien $\Re(\rho) = 1/2$.
\end{proof}
...
\begin{proof}
The architecture of the proof revolves around a geometric translation of the spectrum of zeros of the zeta function. By identifying these zeros with the eigenvalues of the monodromy operator on the motivic Hall algebra $\mathcal{H}_{mot}(\mathcal{M})$, the appearance of an asymmetric zero would cause a rupture in the indecomposability of the Galois motive. To prevent any formal divergence related to the singularities of the moduli space, we systematically restrict ourselves to intersection cohomology, thereby transforming the analytical problem into a categorical rigidity problem.

Consider $\mathcal{E}$ the central Galois motive, modeling the functional symmetry of $\zeta$. Let $M = IH^*(\mathcal{M})$ be the associated space of vanishing cycles, on which the monodromy operator $\Phi$ acts. The sequence of motivic fibrations induces a weight filtration $W_\bullet M$.

Suppose for the sake of contradiction the existence of an asymmetric zero $\rho = \sigma_0 + i \gamma_0$ with $\sigma_0 > 1/2$. By Amitsur duality, this asymmetry corresponds to a component of the motive possessing a non-pure weight, which implies that the monodromy operator $\Phi$ admits an eigenvalue $\lambda$ such that:
\begin{equation*}
    |\lambda| \neq q^{1/2}
\end{equation*}

According to Verma's theorem \cite{verma2026} linking the geometry of the Hall algebra to Auslander-Reiten sequences, there exists a canonical exact sequence in the category of perverse sheaves:
\begin{equation*}
    0 \longrightarrow \tau \mathcal{E} \longrightarrow \mathcal{F} \longrightarrow \mathcal{E} \longrightarrow 0
\end{equation*}
where $\tau$ is the Auslander-Reiten translation. The existence of such a non-unitary eigenvalue forces a topological splitting of this exact sequence in intersection cohomology. The action of $\Phi$ on the extension $\mathcal{F}$ can then be written as a block-diagonal matrix.

By applying the reflection isomorphism $R: \mathcal{H}_{mot}(\mathcal{M}) \xrightarrow{\sim} \mathcal{H}_{mot}(\mathcal{M})$, which reverses the quiver flow, we obtain a compatibility condition on the extensions:
\begin{align*}
    R(\mathcal{F}) &\cong \tau^{-1} \mathcal{F} \\
    \text{Ext}^1(\mathcal{E}, \tau \mathcal{E}) &\xrightarrow{\sim} \text{Ext}^1(\tau^{-1} \mathcal{E}, \mathcal{E})
\end{align*}

However, the presence of the asymmetric eigenvalue $\lambda$, deviating from $q^{1/2}$, induces an asymmetry between the eigenspaces associated with $W_k$ and $W_{-k}$ under the action of $R$. Thus, the intersection form:
\begin{equation*}
    \langle \cdot, \cdot \rangle_{IH} : IH^k(\mathcal{M}) \times IH^{-k}(\mathcal{M}) \longrightarrow \mathbb{Q}_{\ell}
\end{equation*}
becomes degenerate on the subspace generated by the eigenvector corresponding to $\lambda$.

This cohomological degeneracy translates to the existence of a non-trivial proper subobject in the derived category, which decomposes the central Galois motive $\mathcal{E}$, originally assumed to be strictly indecomposable, leading to a formal contradiction.

Consequently, we must have $|\lambda| = q^{1/2}$ for any eigenvalue of $\Phi$. The purity constraint on the monodromy operator is therefore preserved, and any zero $\rho$ of $\zeta$ indeed satisfies $\Re(\rho) = 1/2$.
\end{proof}
