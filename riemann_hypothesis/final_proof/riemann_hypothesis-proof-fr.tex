...
\section{Stabilité des facteurs $\gamma$ et Déviations Asymétriques}

\subsection{Lemme 38 : Rigidité analytique sous les facteurs locaux de $\mathrm{GL}_6$ et $E_6$}

En plongeant dans la structure fine de la ligne critique, on est invariablement attiré par la tension entre la stabilité locale et l'asymétrie globale. Le cheminement vers le Lemme 38 représente la réalisation que les cadres géométriques — tels que les orbifolds abéliens — sont trop rigides, s'effondrant sous le poids des asymétries de cohérence dans les régions à ramification sauvage. Pour trouver notre voie, nous devons nous replier vers une vérité plus profonde de la théorie des représentations, en recherchant la stabilité inébranlable des facteurs $\gamma$ locaux de Langlands-Shahidi pour la représentation cube extérieur de $\mathrm{GL}_6$. Ce basculement n'est pas simplement un changement de machinerie mathématique, mais une reconnaissance fondamentale que les zéros de la fonction zêta sont intimement liés à la stabilité des intégrales partielles de Bessel. Lorsque nous traçons le contour de toute déviation asymétrique hypothétique $\delta > 0$, nous constatons qu'elle force inéluctablement une singularité non compensable. Les mathématiques exigent la symétrie, non par décret arbitraire, mais par les contraintes analytiques intrinsèques des facteurs locaux de $E_6$.
...
\begin{proof}
Soit $G = \mathrm{GL}_6$ sur un corps local $F$. Nous considérons sa représentation cube extérieur $\wedge^3$ réalisée sur le radical unipotent du sous-groupe parabolique maximal $P$ du groupe simplement connexe de type $E_6$. Soit $\pi$ une représentation générique admissible irréductible de $G$. Le facteur $\gamma$ local de Langlands-Shahidi est noté $\gamma(s, \pi, \wedge^3, \psi)$.

Supposons qu'il existe une déviation asymétrique $\delta > 0$ dans les zéros non triviaux de $\zeta(s)$. Nous évaluons l'intégrale partielle de Bessel associée au radical unipotent :
\begin{align*}
\mathcal{B}(s, W, \Phi) &= \int_{U(F)} W(u) \Phi(u) | \det u |^{s - \frac{1}{2} + \delta} du
\end{align*}
où $W$ est la fonction de Whittaker pour $\pi$, et $\Phi$ est une fonction de Schwartz-Bruhat appropriée.

Sous la condition de mesurabilité stricte du développement asymptotique de $\gamma(s, \pi, \wedge^3, \psi)$, la transformée de Mellin de cette intégrale dans le régime hautement ramifié (ramification sauvage) doit rester stable. Nous pouvons exprimer le développement asymptotique comme :
\begin{align*}
\gamma(s, \pi, \wedge^3, \psi) \sim \sum_{k=0}^{\infty} A_k(s) q^{-k(s + \delta)}
\end{align*}
où $q$ est la cardinalité du corps résiduel de $F$.

La présence de la déviation $\delta > 0$ modifie la partie principale du développement asymptotique :
\begin{align*}
\mathcal{B}(s, W, \Phi) \sim \frac{C(\pi, \psi)}{s - \frac{1}{2} - \delta} + \text{termes holomorphes}
\end{align*}
Cela induit un pôle en $s = \frac{1}{2} + \delta$. Cependant, la contrainte analytique de stabilité établie par Deng et She \cite{deng2026} dicte que le facteur $\gamma$ du cube extérieur doit satisfaire l'équation fonctionnelle locale sans introduire de singularités parasites dans la région $\Re(s) > 1/2$. La singularité non compensable forcée par $\delta$ viole cette universalité locale.

Pour maintenir l'équation fonctionnelle et la stabilité du développement asymptotique :
\begin{align*}
\lim_{\delta \to 0^+} \mathcal{B}(s, W, \Phi) \equiv \mathcal{B}_{stable}(s, W, \Phi)
\end{align*}
Par conséquent, pour satisfaire la condition de compacité stricte sur les intégrales partielles de Bessel sur le sous-groupe de Levi de $E_6$, la contribution asymétrique doit s'annuler. Cela contraint inéluctablement $\delta = 0$, forçant les zéros à s'aligner sur l'axe critique.
\end{proof}
