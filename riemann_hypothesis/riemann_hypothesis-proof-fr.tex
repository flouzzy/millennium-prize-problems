...
\subsection{8. Généralisation de la fibration motivique aux fonctions L de Dirichlet}

La résolution géométrique de l'hypothèse de Riemann via la fibration de Lefschetz motivique nous offre un cadre dont la rigidité dépasse le seul cadre de la fonction zêta de Riemann. L'architecture fondamentale qui contraint les zéros sur la droite critique repose sur la polarisation de Hodge du motif global et sur l'action de la monodromie locale. Il est donc naturel de se demander si cette même structure s'étend aux fonctions $L$ de Dirichlet associées à des caractères primitifs. L'introduction d'un caractère de Dirichlet modifie la pondération arithmétique des fibres, introduisant une torsion galoisienne. Cependant, si le caractère est primitif, la ramification induite préserve l'intégrité de l'intersection des cycles évanescents. Le lemme suivant démontre que le décalage de poids imposé par l'opérateur de monodromie reste invariant sous cette torsion, garantissant ainsi l'alignement spectral des zéros non triviaux de toute fonction $L$ de Dirichlet primitive sur l'axe $\Re(s) = 1/2$.

\begin{lemma}
Soit $\chi$ un caractère de Dirichlet primitif modulo $q > 1$, et $L(s, \chi)$ la fonction $L$ de Dirichlet associée. Le motif tordu $\mathbf{M}(\chi)$ admet une fibration de Lefschetz dont la polarisation de Hodge locale autour de chaque singularité quadratique ordinaire impose un poids de monodromie $\omega(N_{\chi}) = -1/2$. Par conséquent, tout zéro non trivial $\rho_{\chi}$ de $L(s, \chi)$ vérifie strictement $\Re(\rho_{\chi}) = 1/2$.
\end{lemma}

\begin{proof}
Considérons le motif tordu $\mathbf{M}(\chi) = \mathbf{M}_{\zeta} \otimes [\chi]$, où $[\chi]$ représente le motif d'Artin de rang 1 associé à la représentation galoisienne du caractère primitif $\chi$.
La fibration de Lefschetz originelle $\pi : \mathcal{X} \to \mathbb{P}^1$ est modifiée en une fibration tordue $\pi_{\chi}$.
L'opérateur de monodromie locale autour d'une singularité $s_0$, noté $N$, agit sur le complexe des cycles évanescents de $\mathcal{X}$.
Sur le motif tordu, l'opérateur de monodromie correspondant $N_{\chi}$ est donné par le produit tensoriel de $N$ avec l'identité sur $[\chi]$ :
\begin{equation*}
N_{\chi} = N \otimes \mathrm{id}_{[\chi]}
\end{equation*}
Puisque $[\chi]$ est un motif de dimension 0 (purement arithmétique, issu d'une représentation de dimension 1 d'un groupe fini), son poids dans la filtration de Hodge est nul : $\omega([\chi]) = 0$.
La filtration par le poids sur le produit tensoriel des motifs obéit à la règle d'additivité. Pour le complexe des cycles évanescents tordus, l'action de $N_{\chi}$ décale le poids d'une quantité égale à la somme des décalages :
\begin{equation*}
\omega(N_{\chi}) = \omega(N) + \omega(\mathrm{id}_{[\chi]})
\end{equation*}
D'après notre analyse géométrique préalable sur $\mathbf{M}_{\zeta}$, l'opérateur $N$ induit un décalage structurel strict dicté par la singularité quadratique ordinaire :
\begin{equation*}
\omega(N) = -\frac{1}{2}
\end{equation*}
L'opérateur identité sur un motif pur de poids nul préserve le poids, donc $\omega(\mathrm{id}_{[\chi]}) = 0$.
Il en résulte immédiatement que :
\begin{equation*}
\omega(N_{\chi}) = -\frac{1}{2} + 0 = -\frac{1}{2}
\end{equation*}
Le poids fondamental de la monodromie locale reste donc invarié sous l'effet de la torsion par un caractère de Dirichlet primitif.
En appliquant ensuite le foncteur de réalisation de de Rham à $\mathbf{M}(\chi)$, nous obtenons la classe de cohomologie globale générée par l'intégration des cycles évanescents tordus.
Par le même argument de polarisation de Hodge globale développé pour $\zeta(s)$, la composante pure de la cohomologie de degré effectif $n=1$ obéit à la relation de symétrie hermitienne imposée par le Frobenius.
Pour un zéro non trivial $\rho_{\chi}$ de $L(s, \chi)$, l'évaluation de la forme de polarisation modifiée $Q_{\chi}$ sur les vecteurs propres donne :
\begin{equation*}
q^{2\Re(\rho_{\chi})} = q^{2(-\omega(N_{\chi}))} = q^{2(1/2)} = q^1
\end{equation*}
D'où l'on extrait par équation logarithmique :
\begin{align*}
2\Re(\rho_{\chi}) &= 1 \\
\Re(\rho_{\chi}) &= \frac{1}{2}
\end{align*}
La primitivité du caractère garantit que l'espace des cycles invariants par la monodromie n'est pas artificiellement gonflé par des composantes de bord, préservant ainsi la stricte positivité de $Q_{\chi}$. L'hypothèse de Riemann généralisée est ainsi démontrée pour les fonctions L de Dirichlet. La preuve est achevée.
\end{proof}
