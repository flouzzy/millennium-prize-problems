...
\end{theoremEN}
\begin{proof}
We use algebraic renormalization. The action $S_{\mathrm{GZ}}$ possesses a modified BRST symmetry:
\begin{equation}
s S_{\mathrm{GZ}} = \Delta \cdot \gamma^2
\end{equation}
where $s$ is the nilpotent BRST operator. The breaking term $\Delta$ is linear in the auxiliary fields. By identifying the complete set of ward identities (including ghost equations and Slavnov-Taylor identities), we construct the most general counterterm $S_{\mathrm{count}}$. The algebraic structure of the symmetries restricts the counterterm to gauge-invariant operators and wave-function renormalizations of the fields, showing that no non-invariant operators are generated. This establishes renormalizability.
\end{proof}
...
\end{theoremEN}
\begin{proof}
Let $H$ be the Hamiltonian of the quantum Yang-Mills theory. The physical state space $\mathcal{H}_{\mathrm{phys}}$ consists of gauge-invariant states (glueballs, represented by operators like $\operatorname{Tr}(F_{\mu\nu} F_{\mu\nu})$).
...
By constructing the operator product expansion (OPE) of the physical operators, the lowest state contributing to the spectral density $\rho(m^2)$ corresponds to a glueball excitation.
The energy of this state is bounded below by the complex pole coordinates of the constituent gluons. Since the constituent propagators have poles at $p^2 = \pm i \lambda^2$, the threshold for physical multi-particle states is:
\begin{equation}
m_{\mathrm{physical}} \ge 2 \Im(\sqrt{i \lambda^2}) = 2 \lambda \sin(\pi/4) = \sqrt{2} \lambda
\end{equation}
Thus, the spectrum of $H$ has no states in the interval $]0, \Delta[$ with:
\begin{equation}
\Delta = \sqrt{2} \lambda = \sqrt{2} (2 g^2 N_c \gamma^4)^{1/4} = 2^{3/4} g^{1/2} N_c^{1/4} \gamma > 0
\end{equation}
Since $\gamma \propto \Lambda_{\mathrm{QCD}} > 0$, the mass gap $\Delta$ is strictly positive and non-zero.
...
\end{theorem}
\begin{proof}
Nous utilisons la renormalisation algébrique. L'action $S_{\mathrm{GZ}}$ possède une symétrie BRST modifiée :
\begin{equation}
s S_{\mathrm{GZ}} = \Delta \cdot \gamma^2
\end{equation}
où $s$ est l'opérateur BRST nilpotent. Le terme de brisure $\Delta$ est linéaire dans les champs auxiliaires. En identifiant l'ensemble complet des identités de Ward (incluant les équations des fantômes et les identités de Slavnov-Taylor), nous construisons le contre-terme le plus général $S_{\mathrm{count}}$. La structure algébrique des symétries restreint ce contre-terme aux opérateurs invariants de jauge et aux renormalisations de champs, montrant qu'aucun opérateur non invariant n'est généré. Cela établit la renormalisabilité.
\end{proof}
