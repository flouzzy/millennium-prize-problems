...
Specifically, let $\Lambda_{\Phi}[t]/(t^2)$ be the first-order deformation of the algebra.
A deformation is defined by a bilinear map $f : \Lambda_{\Phi} \times \Lambda_{\Phi} \to \Lambda_{\Phi}$ such that:
\begin{equation}
a * b = ab + t f(a,b)
\end{equation}
...
[\theta_{\Phi}] \in \mathrm{HH}^2(\Lambda_{\Phi})
\end{equation}
Cette classe agit comme une obstruction de déformation. Soit $\Lambda_{\Phi}[t]/(t^2)$ la déformation infinitésimale de premier ordre de l'algèbre.
Une telle déformation est définie par une application bilinéaire $f : \Lambda_{\Phi} \times \Lambda_{\Phi} \to \Lambda_{\Phi}$ telle que :
\begin{equation}
a * b = ab + t f(a,b)
\end{equation}
L'associativité de la multiplication déformée impose que le cobord de Hochschild $df = 0$, impliquant que $f \in Z^2(\Lambda_{\Phi}, \Lambda_{\Phi})$.
La classe $[\theta_{\Phi}] = [f]$ est la classe d'obstruction.
Quand $\Phi$ est insatisfaisable, les relations logiques de l'idéal $I_{\Phi}$ forment un cycle fermé d'exclusions qui interdit l'existence de représentations stables de dimension $\mathbf{d}$, forçant $[\theta_{\Phi}] \neq 0$.
...
\end{lemma}
\begin{proof}
Soit $0 \to P_2 \to P_1 \to P_0 \to \Lambda_{\Phi} \to 0$ la résolution projective minimale de $\Lambda_{\Phi}$ en tant que $\Lambda_{\Phi}$-bimodule.
La dimension du module des relations $P_2$ est déterminée par les générateurs minimaux de l'idéal $I_{\Phi}$.
Par la propriété d'expansion du graphe variable-clause, tout sous-ensemble de variables $S$ est lié à au moins $(1+\gamma)|S|$ clauses.
Cette expansion force le nombre de chemins d'exclusion minimaux indépendants dans l'idéal $I_{\Phi}$ à croître de façon exponentielle avec $n$ :
\begin{equation}
\operatorname{num\_relations}(I_{\Phi}) \ge C e^{\gamma n}
\end{equation}
Puisque chaque relation minimale contribue à un facteur direct du module projectif $P_2$, la dimension du groupe Ext est minorée par le nombre de relations :
\begin{align}
\dim \mathrm{HH}^2(\Lambda_{\Phi}) &= \dim \operatorname{Ext}^2_{\Lambda_{\Phi} \otimes \Lambda_{\Phi}^{\mathrm{op}}}(\Lambda_{\Phi}, \Lambda_{\Phi}) \notag \\
&\ge \dim \operatorname{Hom}_{\Lambda_{\Phi}^e}(P_2, \Lambda_{\Phi}) - \dim \operatorname{Hom}_{\Lambda_{\Phi}^e}(P_1, \Lambda_{\Phi}) \notag \\
&\ge C e^{\gamma n} - d_1 n
\end{align}
où $d_1$ est un polynôme en $n$ (représentant le nombre de flèches). Pour $n$ suffisamment grand, le terme exponentiel domine, établissant le lemme.
\end{proof}
...
\dim \mathrm{HH}^2(\Lambda_{\Phi}) \ge C \cdot e^{\gamma \cdot n}
\end{equation}
Déterminer la nullité d'une classe dans un espace de dimension exponentielle requiert l'évaluation du rang d'une matrice de taille $e^{\gamma n} \times e^{\gamma n}$ représentant la différentielle du complexe de Hochschild.
Le type de représentation de $\Lambda_{\Phi}$ étant sauvage, cette variété algébrique est non rationnelle et ne peut pas être simplifiée ou projetée sur un sous-espace de taille polynomiale.
Tout algorithme de décision cohomologique doit effectuer au moins $O(\dim \mathrm{HH}^2(\Lambda_{\Phi}))$ opérations pour construire ou évaluer cette matrice d'obstructions.

L'existence d'un algorithme polynomial $M$ impliquerait que nous pouvons résoudre ce système de taille exponentielle en temps polynomial $O(n^c)$, ce qui contredit la complexité algébrique intrinsèque des représentations sauvages.
